// NOTE: This file was generated by the Mojo bindings generator.
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunused-private-field"
#endif

#include "examples/mybench/echo.mojom-sync.h"

#include <math.h>
#include <ostream>
#include <string>
#include <utility>

#include "mojo/public/cpp/bindings/lib/array_serialization.h"
#include "mojo/public/cpp/bindings/lib/bindings_serialization.h"
#include "mojo/public/cpp/bindings/lib/bounds_checker.h"
#include "mojo/public/cpp/bindings/lib/map_data_internal.h"
#include "mojo/public/cpp/bindings/lib/map_serialization.h"
#include "mojo/public/cpp/bindings/lib/message_builder.h"
#include "mojo/public/cpp/bindings/lib/message_validation.h"
#include "mojo/public/cpp/bindings/lib/string_serialization.h"
#include "mojo/public/cpp/bindings/lib/synchronous_connector.h"
#include "mojo/public/cpp/bindings/lib/validate_params.h"
#include "mojo/public/cpp/bindings/lib/validation_errors.h"
#include "mojo/public/cpp/bindings/lib/validation_util.h"
#include "mojo/public/cpp/bindings/message.h"
#include "mojo/public/cpp/bindings/message_validator.h"
#include "mojo/public/cpp/environment/logging.h"
#include "mojo/public/interfaces/bindings/interface_control_messages.mojom.h"
#include "examples/mybench/echo.mojom-internal.h"
#include "examples/mybench/echo.mojom-common.h"
namespace mojo {
namespace examples {

// --- Interface definitions ---
Echo_SynchronousProxy::Echo_SynchronousProxy(
      mojo::internal::SynchronousConnector* connector,
      mojo::internal::MessageValidatorList&& validators)
          : connector_(connector), validators_(std::move(validators)) {
}


bool Echo_SynchronousProxy::EchoString(uint32_t in_value, uint32_t* out_value) {
  size_t size = sizeof(internal::Echo_EchoString_Params_Data);

  auto msg_name = static_cast<uint32_t>(internal::Echo_Base::MessageOrdinals::EchoString);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Echo_EchoString_Params_Data* out_params =
      internal::Echo_EchoString_Params_Data::New(builder.buffer());
  out_params->value = in_value;
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  mojo::Message response_msg;
  if (!connector_->BlockingRead(&response_msg))
    return false; 
  
  // Validate the incoming message.
  std::string response_err;
  if (mojo::internal::RunValidatorsOnMessage(validators_, &response_msg,
                                             &response_err)
        != mojo::internal::ValidationError::NONE) {
    MOJO_LOG(WARNING) << response_err;
    return false;
  } 
  if (mojo::internal::ControlMessageHandler::IsControlMessage(&response_msg)) {
    MOJO_LOG(ERROR) << "Synchronous interface bindings currently don't support "
                       "interface control messages.";
    return false;
  }
  if (response_msg.name() != msg_name) {
    MOJO_LOG(ERROR) << "Expecting response for message = " << msg_name <<
                       ", but received message = " << response_msg.name();
    return false;
  }
  
  internal::Echo_EchoString_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Echo_EchoString_ResponseParams_Data*>(
          response_msg.mutable_payload());
  response_params->DecodePointersAndHandles(response_msg.mutable_handles());
  
  do {
    // NOTE: The memory backing |response_params| may has be smaller than
    // |sizeof(*response_params)| if the message comes from an older version.
    (*out_value) = response_params->value;
  } while (false);
  return true;
}
bool Echo_SynchronousProxy::BuildObject(int64_t in_object_id, uint64_t in_size, mojo::ScopedSharedBufferHandle* out_buffer) {
  size_t size = sizeof(internal::Echo_BuildObject_Params_Data);

  auto msg_name = static_cast<uint32_t>(internal::Echo_Base::MessageOrdinals::BuildObject);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Echo_BuildObject_Params_Data* out_params =
      internal::Echo_BuildObject_Params_Data::New(builder.buffer());
  out_params->object_id = in_object_id;
  out_params->size = in_size;
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  mojo::Message response_msg;
  if (!connector_->BlockingRead(&response_msg))
    return false; 
  
  // Validate the incoming message.
  std::string response_err;
  if (mojo::internal::RunValidatorsOnMessage(validators_, &response_msg,
                                             &response_err)
        != mojo::internal::ValidationError::NONE) {
    MOJO_LOG(WARNING) << response_err;
    return false;
  } 
  if (mojo::internal::ControlMessageHandler::IsControlMessage(&response_msg)) {
    MOJO_LOG(ERROR) << "Synchronous interface bindings currently don't support "
                       "interface control messages.";
    return false;
  }
  if (response_msg.name() != msg_name) {
    MOJO_LOG(ERROR) << "Expecting response for message = " << msg_name <<
                       ", but received message = " << response_msg.name();
    return false;
  }
  
  internal::Echo_BuildObject_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Echo_BuildObject_ResponseParams_Data*>(
          response_msg.mutable_payload());
  response_params->DecodePointersAndHandles(response_msg.mutable_handles());
  
  do {
    // NOTE: The memory backing |response_params| may has be smaller than
    // |sizeof(*response_params)| if the message comes from an older version.
    (*out_buffer).reset(mojo::internal::FetchAndReset(&response_params->buffer));
  } while (false);
  return true;
}
bool Echo_SynchronousProxy::ListObjects(mojo::Array<ObjectInfoPtr>* out_info) {
  size_t size = sizeof(internal::Echo_ListObjects_Params_Data);

  auto msg_name = static_cast<uint32_t>(internal::Echo_Base::MessageOrdinals::ListObjects);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Echo_ListObjects_Params_Data* out_params =
      internal::Echo_ListObjects_Params_Data::New(builder.buffer());
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  mojo::Message response_msg;
  if (!connector_->BlockingRead(&response_msg))
    return false; 
  
  // Validate the incoming message.
  std::string response_err;
  if (mojo::internal::RunValidatorsOnMessage(validators_, &response_msg,
                                             &response_err)
        != mojo::internal::ValidationError::NONE) {
    MOJO_LOG(WARNING) << response_err;
    return false;
  } 
  if (mojo::internal::ControlMessageHandler::IsControlMessage(&response_msg)) {
    MOJO_LOG(ERROR) << "Synchronous interface bindings currently don't support "
                       "interface control messages.";
    return false;
  }
  if (response_msg.name() != msg_name) {
    MOJO_LOG(ERROR) << "Expecting response for message = " << msg_name <<
                       ", but received message = " << response_msg.name();
    return false;
  }
  
  internal::Echo_ListObjects_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Echo_ListObjects_ResponseParams_Data*>(
          response_msg.mutable_payload());
  response_params->DecodePointersAndHandles(response_msg.mutable_handles());
  
  do {
    // NOTE: The memory backing |response_params| may has be smaller than
    // |sizeof(*response_params)| if the message comes from an older version.
    Deserialize_(response_params->info.ptr, &(*out_info));
  } while (false);
  return true;
}
}  // namespace examples
}  // namespace mojo

#if defined(__clang__)
#pragma clang diagnostic pop
#endif
