...
#include "mojo/services/location/interfaces/location_service.mojom-sync.h"
...
LocationService_SynchronousProxy::LocationService_SynchronousProxy(
...
bool LocationService_SynchronousProxy::GetNextLocation(LocationService::UpdatePriority in_priority, mojo::LocationPtr* out_location) {
  size_t size = sizeof(internal::LocationService_GetNextLocation_Params_Data);

  auto msg_name = static_cast<uint32_t>(internal::LocationService_Base::MessageOrdinals::GetNextLocation);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::LocationService_GetNextLocation_Params_Data* out_params =
  ...
  internal::LocationService_GetNextLocation_ResponseParams_Data*
      response_params = reinterpret_cast<internal::LocationService_GetNextLocation_ResponseParams_Data*>(
  ...
  do {
    ...
    if (response_params->location.ptr) {
      (*out_location) = mojo::Location::New();
      Deserialize_(response_params->location.ptr, (*out_location).get());
    }
  } while (false);
  return true;
}
