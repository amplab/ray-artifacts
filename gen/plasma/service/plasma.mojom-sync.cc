// NOTE: This file was generated by the Mojo bindings generator.
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunused-private-field"
#endif

#include "plasma/service/plasma.mojom-sync.h"

#include <math.h>
#include <ostream>
#include <string>
#include <utility>

#include "mojo/public/cpp/bindings/lib/array_serialization.h"
#include "mojo/public/cpp/bindings/lib/bindings_serialization.h"
#include "mojo/public/cpp/bindings/lib/bounds_checker.h"
#include "mojo/public/cpp/bindings/lib/map_data_internal.h"
#include "mojo/public/cpp/bindings/lib/map_serialization.h"
#include "mojo/public/cpp/bindings/lib/message_builder.h"
#include "mojo/public/cpp/bindings/lib/message_validation.h"
#include "mojo/public/cpp/bindings/lib/string_serialization.h"
#include "mojo/public/cpp/bindings/lib/synchronous_connector.h"
#include "mojo/public/cpp/bindings/lib/validate_params.h"
#include "mojo/public/cpp/bindings/lib/validation_errors.h"
#include "mojo/public/cpp/bindings/lib/validation_util.h"
#include "mojo/public/cpp/bindings/message.h"
#include "mojo/public/cpp/bindings/message_validator.h"
#include "mojo/public/cpp/environment/logging.h"
#include "mojo/public/interfaces/bindings/interface_control_messages.mojom.h"
#include "plasma/service/plasma.mojom-internal.h"
#include "plasma/service/plasma.mojom-common.h"
namespace plasma {
namespace service {

// --- Interface definitions ---
Plasma_SynchronousProxy::Plasma_SynchronousProxy(
      mojo::internal::SynchronousConnector* connector,
      mojo::internal::MessageValidatorList&& validators)
          : connector_(connector), validators_(std::move(validators)) {
}


bool Plasma_SynchronousProxy::CreateObject(const mojo::String& in_object_id, uint64_t in_size, const mojo::String& in_name, int64_t in_creator_id, mojo::ScopedSharedBufferHandle* out_buffer) {
  size_t size = sizeof(internal::Plasma_CreateObject_Params_Data);
  size += GetSerializedSize_(in_object_id);
  size += GetSerializedSize_(in_name);

  auto msg_name = static_cast<uint32_t>(internal::Plasma_Base::MessageOrdinals::CreateObject);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Plasma_CreateObject_Params_Data* out_params =
      internal::Plasma_CreateObject_Params_Data::New(builder.buffer());
  SerializeString_(in_object_id, builder.buffer(), &out_params->object_id.ptr);
  if (!out_params->object_id.ptr) {
    MOJO_INTERNAL_DLOG_SERIALIZATION_WARNING(mojo::internal::ValidationError::UNEXPECTED_NULL_POINTER, "null object_id in {{interface.name}}::{{method.name}}");
  }
  out_params->size = in_size;
  SerializeString_(in_name, builder.buffer(), &out_params->name.ptr);
  if (!out_params->name.ptr) {
    MOJO_INTERNAL_DLOG_SERIALIZATION_WARNING(mojo::internal::ValidationError::UNEXPECTED_NULL_POINTER, "null name in {{interface.name}}::{{method.name}}");
  }
  out_params->creator_id = in_creator_id;
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  mojo::Message response_msg;
  if (!connector_->BlockingRead(&response_msg))
    return false; 
  
  // Validate the incoming message.
  std::string response_err;
  if (mojo::internal::RunValidatorsOnMessage(validators_, &response_msg,
                                             &response_err)
        != mojo::internal::ValidationError::NONE) {
    MOJO_LOG(WARNING) << response_err;
    return false;
  } 
  if (mojo::internal::ControlMessageHandler::IsControlMessage(&response_msg)) {
    MOJO_LOG(ERROR) << "Synchronous interface bindings currently don't support "
                       "interface control messages.";
    return false;
  }
  if (response_msg.name() != msg_name) {
    MOJO_LOG(ERROR) << "Expecting response for message = " << msg_name <<
                       ", but received message = " << response_msg.name();
    return false;
  }
  
  internal::Plasma_CreateObject_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Plasma_CreateObject_ResponseParams_Data*>(
          response_msg.mutable_payload());
  response_params->DecodePointersAndHandles(response_msg.mutable_handles());
  
  do {
    // NOTE: The memory backing |response_params| may has be smaller than
    // |sizeof(*response_params)| if the message comes from an older version.
    (*out_buffer).reset(mojo::internal::FetchAndReset(&response_params->buffer));
  } while (false);
  return true;
}
bool Plasma_SynchronousProxy::ResizeObject(const mojo::String& in_object_id, uint64_t in_new_size, mojo::ScopedSharedBufferHandle* out_buffer) {
  size_t size = sizeof(internal::Plasma_ResizeObject_Params_Data);
  size += GetSerializedSize_(in_object_id);

  auto msg_name = static_cast<uint32_t>(internal::Plasma_Base::MessageOrdinals::ResizeObject);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Plasma_ResizeObject_Params_Data* out_params =
      internal::Plasma_ResizeObject_Params_Data::New(builder.buffer());
  SerializeString_(in_object_id, builder.buffer(), &out_params->object_id.ptr);
  if (!out_params->object_id.ptr) {
    MOJO_INTERNAL_DLOG_SERIALIZATION_WARNING(mojo::internal::ValidationError::UNEXPECTED_NULL_POINTER, "null object_id in {{interface.name}}::{{method.name}}");
  }
  out_params->new_size = in_new_size;
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  mojo::Message response_msg;
  if (!connector_->BlockingRead(&response_msg))
    return false; 
  
  // Validate the incoming message.
  std::string response_err;
  if (mojo::internal::RunValidatorsOnMessage(validators_, &response_msg,
                                             &response_err)
        != mojo::internal::ValidationError::NONE) {
    MOJO_LOG(WARNING) << response_err;
    return false;
  } 
  if (mojo::internal::ControlMessageHandler::IsControlMessage(&response_msg)) {
    MOJO_LOG(ERROR) << "Synchronous interface bindings currently don't support "
                       "interface control messages.";
    return false;
  }
  if (response_msg.name() != msg_name) {
    MOJO_LOG(ERROR) << "Expecting response for message = " << msg_name <<
                       ", but received message = " << response_msg.name();
    return false;
  }
  
  internal::Plasma_ResizeObject_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Plasma_ResizeObject_ResponseParams_Data*>(
          response_msg.mutable_payload());
  response_params->DecodePointersAndHandles(response_msg.mutable_handles());
  
  do {
    // NOTE: The memory backing |response_params| may has be smaller than
    // |sizeof(*response_params)| if the message comes from an older version.
    (*out_buffer).reset(mojo::internal::FetchAndReset(&response_params->buffer));
  } while (false);
  return true;
}
bool Plasma_SynchronousProxy::SealObject(const mojo::String& in_object_id) const  {
  size_t size = sizeof(internal::Plasma_SealObject_Params_Data);
  size += GetSerializedSize_(in_object_id);

  auto msg_name = static_cast<uint32_t>(internal::Plasma_Base::MessageOrdinals::SealObject);
  mojo::MessageBuilder builder(msg_name, size);

  internal::Plasma_SealObject_Params_Data* out_params =
      internal::Plasma_SealObject_Params_Data::New(builder.buffer());
  SerializeString_(in_object_id, builder.buffer(), &out_params->object_id.ptr);
  if (!out_params->object_id.ptr) {
    MOJO_INTERNAL_DLOG_SERIALIZATION_WARNING(mojo::internal::ValidationError::UNEXPECTED_NULL_POINTER, "null object_id in {{interface.name}}::{{method.name}}");
  }
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  return true;
}
bool Plasma_SynchronousProxy::GetObject(const mojo::String& in_object_id, bool in_block, mojo::ScopedSharedBufferHandle* out_buffer, uint64_t* out_size) {
  size_t size = sizeof(internal::Plasma_GetObject_Params_Data);
  size += GetSerializedSize_(in_object_id);

  auto msg_name = static_cast<uint32_t>(internal::Plasma_Base::MessageOrdinals::GetObject);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Plasma_GetObject_Params_Data* out_params =
      internal::Plasma_GetObject_Params_Data::New(builder.buffer());
  SerializeString_(in_object_id, builder.buffer(), &out_params->object_id.ptr);
  if (!out_params->object_id.ptr) {
    MOJO_INTERNAL_DLOG_SERIALIZATION_WARNING(mojo::internal::ValidationError::UNEXPECTED_NULL_POINTER, "null object_id in {{interface.name}}::{{method.name}}");
  }
  out_params->block = in_block;
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  mojo::Message response_msg;
  if (!connector_->BlockingRead(&response_msg))
    return false; 
  
  // Validate the incoming message.
  std::string response_err;
  if (mojo::internal::RunValidatorsOnMessage(validators_, &response_msg,
                                             &response_err)
        != mojo::internal::ValidationError::NONE) {
    MOJO_LOG(WARNING) << response_err;
    return false;
  } 
  if (mojo::internal::ControlMessageHandler::IsControlMessage(&response_msg)) {
    MOJO_LOG(ERROR) << "Synchronous interface bindings currently don't support "
                       "interface control messages.";
    return false;
  }
  if (response_msg.name() != msg_name) {
    MOJO_LOG(ERROR) << "Expecting response for message = " << msg_name <<
                       ", but received message = " << response_msg.name();
    return false;
  }
  
  internal::Plasma_GetObject_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Plasma_GetObject_ResponseParams_Data*>(
          response_msg.mutable_payload());
  response_params->DecodePointersAndHandles(response_msg.mutable_handles());
  
  do {
    // NOTE: The memory backing |response_params| may has be smaller than
    // |sizeof(*response_params)| if the message comes from an older version.
    (*out_buffer).reset(mojo::internal::FetchAndReset(&response_params->buffer));
    (*out_size) = response_params->size;
  } while (false);
  return true;
}
bool Plasma_SynchronousProxy::ListObjects(mojo::Array<ObjectInfoPtr>* out_info) {
  size_t size = sizeof(internal::Plasma_ListObjects_Params_Data);

  auto msg_name = static_cast<uint32_t>(internal::Plasma_Base::MessageOrdinals::ListObjects);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Plasma_ListObjects_Params_Data* out_params =
      internal::Plasma_ListObjects_Params_Data::New(builder.buffer());
  out_params->EncodePointersAndHandles(builder.message()->mutable_handles());
  
  if (!connector_->Write(builder.message()))
    return false;
  mojo::Message response_msg;
  if (!connector_->BlockingRead(&response_msg))
    return false; 
  
  // Validate the incoming message.
  std::string response_err;
  if (mojo::internal::RunValidatorsOnMessage(validators_, &response_msg,
                                             &response_err)
        != mojo::internal::ValidationError::NONE) {
    MOJO_LOG(WARNING) << response_err;
    return false;
  } 
  if (mojo::internal::ControlMessageHandler::IsControlMessage(&response_msg)) {
    MOJO_LOG(ERROR) << "Synchronous interface bindings currently don't support "
                       "interface control messages.";
    return false;
  }
  if (response_msg.name() != msg_name) {
    MOJO_LOG(ERROR) << "Expecting response for message = " << msg_name <<
                       ", but received message = " << response_msg.name();
    return false;
  }
  
  internal::Plasma_ListObjects_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Plasma_ListObjects_ResponseParams_Data*>(
          response_msg.mutable_payload());
  response_params->DecodePointersAndHandles(response_msg.mutable_handles());
  
  do {
    // NOTE: The memory backing |response_params| may has be smaller than
    // |sizeof(*response_params)| if the message comes from an older version.
    Deserialize_(response_params->info.ptr, &(*out_info));
  } while (false);
  return true;
}
}  // namespace service
}  // namespace plasma

#if defined(__clang__)
#pragma clang diagnostic pop
#endif
