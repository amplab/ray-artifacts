...
// For:
//     mojo/public/interfaces/bindings/tests/versioning_test_service.mojom
//

package versioning_test_service
...
func (s *Date) Encode(encoder *bindings.Encoder) error {
	encoder.StartStruct(8, 0)
	if err := encoder.WriteUint16(s.Year); err != nil {
		return err
	}
	if err := encoder.WriteUint8(s.Month); err != nil {
		return err
	}
	if err := encoder.WriteUint8(s.Day); err != nil {
		return err
	}
	if err := encoder.Finish(); err != nil {
		return err
...
var date_Versions []bindings.DataHeader = []bindings.DataHeader{
	bindings.DataHeader{16, 0},
}

func (s *Date) Decode(decoder *bindings.Decoder) error {
	header, err := decoder.StartStruct()
	if err != nil {
		return err
	}
	index := sort.Search(len(date_Versions), func(i int) bool {
		return date_Versions[i].ElementsOrVersion >= header.ElementsOrVersion
	})
	if index < len(date_Versions) {
		if date_Versions[index].ElementsOrVersion > header.ElementsOrVersion {
			index--
		}
		expectedSize := date_Versions[index].Size
		if expectedSize != header.Size {
			return &bindings.ValidationError{bindings.UnexpectedStructHeader,
				fmt.Sprintf("invalid struct header size: should be %d, but was %d", expectedSize, header.Size),
			}
...
func (s *Employee) Encode(encoder *bindings.Encoder) error {
	...
	if s.Birthday == nil {
		...
		if err := (*s.Birthday).Encode(encoder); err != nil {
			return err
		}
	}
	if err := encoder.Finish(); err != nil {
...
func (s *Employee) Decode(decoder *bindings.Decoder) error {
	...
	if header.ElementsOrVersion >= 1 {
		...
		if pointer0 == 0 {
			...
			s.Birthday = new(Date)
			if err := (*s.Birthday).Decode(decoder); err != nil {
				return err
			}
		}
	}
