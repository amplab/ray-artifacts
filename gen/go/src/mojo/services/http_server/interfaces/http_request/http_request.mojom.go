...
func (s *HttpRequest) Encode(encoder *bindings.Encoder) error {
	...
	if err := encoder.WriteString(s.RelativeUrl); err != nil {
		return err
	}
	if err := encoder.WritePointer(); err != nil {
		return err
	...
	if s.Headers == nil {
		...
		encoder.StartMap()
		{
			...
			if encoder.Deterministic() {
			  for key0, _ := range (*s.Headers) {
				keys0 = append(keys0, key0)
			  }
			  bindings.SortMapKeys(&keys0)
			  values0 = make([]string, len(keys0))
			  for i, key := range keys0 {
			    values0[i] = (*s.Headers)[key]
			  }
			} else {
			  for key0, value0 := range (*s.Headers) {
				keys0 = append(keys0, key0)
...
func (s *HttpRequest) Decode(decoder *bindings.Decoder) error {
	...
	if header.ElementsOrVersion >= 0 {
		...
		if pointer0 == 0 {
			...
			s.Headers = new(map[string]string)
