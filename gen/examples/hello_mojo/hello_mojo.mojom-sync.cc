...
#include "examples/hello_mojo/hello_mojo.mojom-sync.h"
...
HelloMojo_SynchronousProxy::HelloMojo_SynchronousProxy(
...
bool HelloMojo_SynchronousProxy::Say(const mojo::String& in_request, mojo::String* out_response) {
  size_t size = sizeof(internal::HelloMojo_Say_Params_Data);
  size += GetSerializedSize_(in_request);

  auto msg_name = static_cast<uint32_t>(internal::HelloMojo_Base::MessageOrdinals::Say);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::HelloMojo_Say_Params_Data* out_params =
      internal::HelloMojo_Say_Params_Data::New(builder.buffer());
  SerializeString_(in_request, builder.buffer(), &out_params->request.ptr);
  ...
  internal::HelloMojo_Say_ResponseParams_Data*
      response_params = reinterpret_cast<internal::HelloMojo_Say_ResponseParams_Data*>(
  ...
  do {
    ...
    Deserialize_(response_params->response.ptr, &(*out_response));
  } while (false);
  return true;
}
}  // namespace examples
