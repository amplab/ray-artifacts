...
#include "shell/test/pingable.mojom-sync.h"
...
Pingable_SynchronousProxy::Pingable_SynchronousProxy(
...
bool Pingable_SynchronousProxy::Ping(const mojo::String& in_message, mojo::String* out_app_url, mojo::String* out_connection_url, mojo::String* out_message) {
  size_t size = sizeof(internal::Pingable_Ping_Params_Data);
  size += GetSerializedSize_(in_message);

  auto msg_name = static_cast<uint32_t>(internal::Pingable_Base::MessageOrdinals::Ping);
  mojo::RequestMessageBuilder builder(msg_name, size);

  internal::Pingable_Ping_Params_Data* out_params =
  ...
  internal::Pingable_Ping_ResponseParams_Data*
      response_params = reinterpret_cast<internal::Pingable_Ping_ResponseParams_Data*>(
  ...
  do {
    ...
    Deserialize_(response_params->app_url.ptr, &(*out_app_url));
    Deserialize_(response_params->connection_url.ptr, &(*out_connection_url));
    Deserialize_(response_params->message.ptr, &(*out_message));
  } while (false);
  return true;
}
